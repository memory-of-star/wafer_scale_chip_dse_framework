...
def process_multi_fidelity(points_dict, threshold = 0, queue : Queue = None, design_space=None, design_points=None, **optimizer_kwargs):
...
def multi_fidelity_double_circulation_search(model_num = 1, run_times = 20, max_runs = 100, initial_runs = 6, run_name = 'result', **kwargs):
    design_points, design_space = build_search_space()
    ...
    for k in range(run_times):
        for i in range(model_num):
            ...
            evaluator.choose_model = i
            p = Process(target=process_multi_fidelity, args=(points_dic2, -1e11, queue_gp, design_space, design_points), kwargs=optimizer_kwargs)
            p.start()
            pool.append(p)
        ...
        #                     }
        #     evaluator.choose_model = i
        #     p = Process(target=process_multi_fidelity, args=(points_dic, -1e11, queue_random, design_space, design_points), kwargs=optimizer_kwargs)
        #     p.start()
        #     pool.append(p)
    ...
    plot.plot_curve(curve_2, curve_1, path=os.path.join(root_path, 'result/picture', run_name+'_curve.png'))
    # plot.plot_hist(np.array([curve_random]), np.array([curve_gp]), path=os.path.join(root_path, 'result/picture', run_name+'_hist.png'))
...
if __name__ == "__main__":
    ...
    # print(internal_points[0])

    # func_multi_fidelity_with_inner_search(internal_points[0])

    multi_fidelity_double_circulation_search(run_times=2, max_runs=20, run_name='try')