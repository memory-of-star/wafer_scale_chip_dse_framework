...
def sampling_configuration_factory(space, points_dict):
    _space = space
    def sample_configuration(size):
        ...
        if size == 1:
            while True:
                ...
                design_point, model_parallel = fine_tune_model_para(design_point, model_parallel)
            ...
            return Configuration(_space, design_point)
        else:
            ret = []
            for i in range(size):
                ...
                design_point.update(model_parallel)
                ret.append(Configuration(_space, design_point))
...
def generate_legal_points(num = 100, _choose_model=-1):
    ...
    sampling_func = sampling_configuration_factory(space=space, points_dict=points_dict)
